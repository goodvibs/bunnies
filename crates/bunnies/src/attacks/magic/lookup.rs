...
pub static ROOK_MAGIC_ATTACKS_LOOKUP: LazyLock<MagicAttacksLookup> = LazyLock::new(|| {
    MagicAttacksLookup::load_or_generate(magic_table_path("rook_magic_attacks_lookup.bin"), || {
        MagicAttacksInitializer::new(3141592653, ROOK_ATTACK_TABLE_SIZE)
            .init_for_piece(ROOK_RELEVANT_MASKS, &manual_single_rook_attacks)
    })
...
pub static BISHOP_MAGIC_ATTACKS_LOOKUP: LazyLock<MagicAttacksLookup> = LazyLock::new(|| {
    MagicAttacksLookup::load_or_generate(
        ...
        || {
            MagicAttacksInitializer::new(0, BISHOP_ATTACK_TABLE_SIZE)
                .init_for_piece(BISHOP_RELEVANT_MASKS, &manual_single_bishop_attacks)
        },
    )
